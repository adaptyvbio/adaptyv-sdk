...
Each test below would have caught one of the high-severity divergences found
when the SDK trailed the live spec: wrong endpoint paths, the cursor-based
update feed, the flat list shapes, and the renamed enum members. They are
respx-mocked and require no network, so they run in the fast suite (neither
``slow`` nor ``integration``). The single-line docstring on each test records
its assumed starting state and the invariant it guards.
...
class TestEndpointPaths:
    ...
    def test_submit_posts_to_submit_path(self, client: FoundryClient) -> None:
        ...
        assert route.called
        assert str(route.calls[0].request.url).endswith(
            f"/api/v1/experiments/{EXP_UUID}/submit"
        )

    @respx.mock
    def test_confirm_quote_posts_to_quote_confirm_path(
        ...
        client.experiments.confirm_quote(
            EXP_UUID, purchase_order_number="PO-42"
        )
    ...
    def test_modify_uses_http_patch(self, client: FoundryClient) -> None:
        ...
        route = respx.patch(f"{BASE}/experiments/{EXP_UUID}").mock(
            return_value=Response(
                200, json={"id": EXP_UUID, "updated": True, "message": "ok"}
            )
        )
...
class TestNewResourceSurfaces:
    ...
    def test_quotes_list_and_actions(self, client: FoundryClient) -> None:
        ...
        list_route = respx.get(f"{BASE}/quotes").mock(
            return_value=Response(
                200, json={"items": [], "total": 0, "count": 0, "offset": 0}
            )
        )
