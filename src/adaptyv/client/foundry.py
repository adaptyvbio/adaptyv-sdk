...
class ExperimentsAPI:
    ...
    def list(
        ...
        *,
        ...
        filter
            Filter expression, e.g. ``eq(status,draft)``. Combine with
            ``and(...)``, ``or(...)``, or ``in(field,a,b)``.
        search
        ...
        *,
        ...
        if exp_type in (ExperimentType.affinity, ExperimentType.screening):
            if not experiment_spec.target_id:
                raise ValidationError(f"{exp_type.value} experiments require target_id")

        # Affinity requires antigen_concentrations
        if exp_type == ExperimentType.affinity:
            if not experiment_spec.antigen_concentrations:
                raise ValidationError("Affinity experiments require antigen_concentrations")
    ...
    def cost_estimate(
        ...
        payload = {"experiment_spec": experiment_spec.model_dump(mode="json", exclude_none=True)}
    ...
    def confirm_quote(
        ...
        *,
        ...
        """
        request = ConfirmQuoteRequest(notes=notes, purchase_order_number=purchase_order_number)
        payload = request.model_dump(mode="json", exclude_none=True)
        response = self._client._post(f"/experiments/{experiment_id}/quote/confirm", payload)
        ...
        *,
        ...
        request = ModifyExpRequest(
            ...
            antigen_concentrations=(
                list(antigen_concentrations) if antigen_concentrations is not None else None
            ),
    ...
    def get_sequences(
        ...
        *,
        ...
        if sort is not None:
            params["sort"] = sort
        response = self._client._get(f"/experiments/{experiment_id}/sequences", params=params)
    ...
    def list(
        ...
        *,
        ...
        limit, offset
            Offset-pagination window.
        filter
            Filter expression; narrow to one experiment with
...
class SequencesAPI:
    ...
    def create(
        ...
        """
        seq_entries = [s if isinstance(s, SequenceEntry) else SequenceEntry(**s) for s in sequences]
        payload = {
            "experiment_code": experiment_code,
            "sequences": [s.model_dump(mode="json", exclude_none=True) for s in seq_entries],
        }
...
class FoundryClient:
    ...
    def _raise_for_status(
        ...
        if response.status_code == 400:
            # Some 400s arrive as {"experiment_id": ""} with the error detail omitted.
            error_msg = body.get("error") or body.get("message") or body.get("detail")
            if not error_msg and body == {"experiment_id": ""}:
                error_msg = (
                    "Request rejected and the API returned no error detail. "
